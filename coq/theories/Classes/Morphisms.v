...
*)
(*temp*)
Require Import Coq.Init.Logic(*Coq.Init.Specif*).
...
Section Proper.
  ...
  Class Proper (R : relation A) (m : A) : Type :=
    proper_prf : R m m.
  ...
  Class ProperProxy (R : relation A) (m : A) : Type :=
    proper_proxy : R m m.

  (*Lemma eq_proper_proxy (x : A) : ProperProxy (@eq A) x.
  Proof. firstorder. Qed.*)

  Definition reflexive_proper_proxy `(H : Reflexive A R) (x : A) : ProperProxy R x
    := H _.

  Definition proper_proper_proxy x `(H : Proper R x) : ProperProxy R x
    := H.
...
*)
CoInductive apply_subrelation : Type := do_subrelation.
(*
...
Section GenericInstances.
  ...
  Program Definition complement_proper
          `(mR : Proper (A -> A -> Type) (RA ==> RA ==> iff) R) :
...
CoInductive normalization_done : Type := did_normalization.
...
Section Normalize.
  ...
  Class Normalizes (m : relation A) (m' : relation A) : Type :=
